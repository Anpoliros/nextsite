...
它不负责 Markdown AST 细节、文章文件扫描、内容同步和 Portal 配置模型。这些能力分别由 `lib/markdown/`、`lib/posts/`、`scripts/` 和 `config/` 提供。文章详情页负责调用 `renderMarkdownDocument()`，并把结果交给正文和目录组件展示。
...
- `app/[category]/page.tsx`：分类页，按文章目录分类筛选，并使用 `contentConfig.categories` 展示分类信息。
...
- 修改分页规则时，同时检查首页、分类页、标签页和 `config/content.ts` 的 `pagination`。
