...
我们来看看现在的markdown渲染实现。我没什么react基础，有几个疑惑的问题

1. 注意到/app中的文件几乎都没有在调用lib/markdown，那怎么实现的渲染呢？

2. 是在构建的时候就渲染完了，还是说客户端拿到文件后会自己渲染整个网页，或者是自己渲染codeblock？

3. codeblock.ts感觉写的十分臃肿，特别是里面构造header的部分。根据我对react的理解，return一个html片段看起来也许更合理？


---

感觉明白了一些，所以说虽然臃肿但是必要且不会影响用户体验。还有几个问题


1. 之前为了解决shiki背景色的问题（shiki背景色不渲染，网页背景色穿透了代码块显示），在global.css中增加了一些代码，这会不会降低可维护性？

2. 发现有两个兼容性问题，1在某些设备上所有css都失效，2在某些设备上HeaderActions和MarkdownEnhancer提供的按钮功能会失效。可能的原因是？



# 0430

我们来为lib/markdown加入表格支持，index.ts识别到表格后，将调用table.ts渲染。

1. 基本
样式只要最简单的就可以，行列都有线条。遵循markdown格式，例如
| 左对齐 | 居中对齐 | 右对齐 |
| :----- | :------: | -----: |
| 单元格1 | 单元格2 | 单元格3 |

2. 表头
将把`| --- | --- |`上面的行加粗，以体现表头。另外如果`| --- | --- |`上面没有内容，也将识别为表格，只不过没有任何行被加粗。
...
3. 折行
表格div有一个类似codeblock的header，但是表格的header本身不显示任何UI，全透明。header的右侧是按钮。同样是折行按钮和复制按钮。
- 折行按钮按下后，表格的每个单元格都将不自动换行，表格将被展开，可以左右划动；再次点击切换状态。
- 复制按钮按下后将复制整个表格的内容，带缩进。