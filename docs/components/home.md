...
`components/home/` 放置首页专用组件。当前核心组件是 `Hero`，它读取 `heroConfig` 渲染首页顶部横幅，不负责文章数据、分页或右侧 Portal 布局。

## 实现视角

- `components/home/Hero.tsx`：Server Component，使用 `next/link` 渲染可选的副标题链接。
- `config/hero.ts`：集中维护 Hero 的背景图、标题、副标题、对齐、颜色和字号。
...
- 修改 Hero 展示内容时，优先改 `config/hero.ts`。
