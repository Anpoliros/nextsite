    ...
    <header className="w-full border-b border-gray-200 dark:border-gray-800">
      <div className={`${siteConfig.layout.headerContainer} mx-auto py-4`}>
        ...
        <div className="flex items-center justify-between">
          ...
          {/* 右侧操作区：深色模式、语言切换、帮助等 */}
          <div className={`flex items-center gap-4 ${siteConfig.ui.fontSizes.headerActions}`}>
            <button className="hover:opacity-70 transition-opacity" title="深色模式">
              [Dark]
            </button>
            <button className="hover:opacity-70 transition-opacity" title="语言切换">
              [Lang]
            </button>
            <button className="hover:opacity-70 transition-opacity" title="帮助">
              [Help]
            </button>
          </div>
        </div>
        ...
        {/* 窄屏第二行：导航链接放在下方并左对齐 */}
        <nav className={`flex md:hidden items-center gap-4 ${siteConfig.ui.fontSizes.headerNav} mt-4 overflow-x-auto pb-1`}>
          {siteConfig.navLinks.map((link) => (
            <Link
              key={link.title}
              href={link.href}
